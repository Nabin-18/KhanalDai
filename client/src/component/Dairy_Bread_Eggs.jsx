...
const Dairy_Bread_Eggs= () => {
  ...
  const handleSeeMore = () => {
    setVisibleItems(allProducts.filter((item) => item.category === "bakery").length);
  };

  return (
    <div className="w-[90%] m-auto  h-fit">
      <div className="flex justify-between p-4 ">
        <h1 className="font-semibold text-xl mx-4">Dairy Bread Eggs</h1>
        {visibleItems < allProducts.filter((item) => item.category === "bakery").length && (
          <h2
