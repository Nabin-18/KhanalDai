...
const PersonalCare = () => {
  ...
  const handleSeeMore = () => {
    setVisibleItems(
      allProducts.filter((item) => item.category === "personal_care").length
    );
  };
  ...
  return (
    <div className="w-[90%] m-auto  h-fit">
      <div className="flex justify-between p-4 ">
        <h1 className="font-semibold text-xl mx-4">Personal Care</h1>
        {visibleItems <
          allProducts.filter((item) => item.category === "personal_care")
            .length && (
          <h2
      ...
      <div className="p-4 grid grid-cols-2 sm:grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
        {allProducts
          .filter((item) => item.category === "personal_care")
