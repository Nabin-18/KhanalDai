...
const Pharmacy= () => {
  ...
  const handleSeeMore = () => {
    setVisibleItems(allProducts.filter((item) => item.category === "pharmacy").length);
  };

  return (
    <div className="w-[90%] m-auto  h-fit">
      <div className="flex justify-between p-4 ">
        <h1 className="font-semibold text-xl mx-4">Pharmacy Products</h1>
        {visibleItems < allProducts.filter((item) => item.category === "pharmacy").length && (
          <h2
